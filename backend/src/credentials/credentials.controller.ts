  ...
  try {
    ...
    if (keyName === 'ai_api_key') {
      try {
        ...
        if (!aiRes.ok) return res.status(400).json({ valid: false, message: 'Invalid OpenAI API Key' });
      } catch (e) {
      ...
      try {
        ...
        if (stripeRes.status === 401) return res.status(400).json({ valid: false, message: 'Invalid Stripe Secret Key' });
      } catch (e) {
