...
export default function BYOKVault() {
  ...
  useEffect(() => {
    const fetchStatus = async () => {
      try {
        const { hasPin } = await apiClient.get(`/api/sites/${siteId}/credentials/pin/status`);
  ...
  const fetchCredentials = async () => {
    try {
      const data = await apiClient.get(`/api/sites/${siteId}/credentials`);
      setCredentials(data);
    } catch (err) {
      console.error(err);
    }
  ...
  const handleSetupPin = async (e: React.FormEvent) => {
    ...
    try {
      await apiClient.post(`/api/sites/${siteId}/credentials/pin/setup`, { pin });
  ...
  const handleUnlock = async (e: React.FormEvent) => {
    ...
    setIsSubmitting(true);
    try {
      await apiClient.post(`/api/sites/${siteId}/credentials/pin/verify`, { pin });
      setViewState('unlocked');
      await fetchCredentials();
      setPin('');
    } catch (err: unknown) {
  ...
  const handleForgotPin = async () => {
    ...
    setIsSubmitting(true);
    try {
      await apiClient.post(`/api/sites/${siteId}/credentials/pin/forgot`, {});
  ...
  const handleResetPin = async (e: React.FormEvent) => {
    ...
    setIsSubmitting(true);
    try {
      await apiClient.post(`/api/sites/${siteId}/credentials/pin/reset`, { otp, newPin });
      setViewState('locked');
      setOtp('');
      setNewPin('');
      setError('');
