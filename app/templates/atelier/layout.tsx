...
function Header({ initialCustomData, basePath }: { initialCustomData?: any, basePath: string }) {
  ...
  const tBrandName = customData?.formData?.brandName || "ATELIER";
  ...
  return (
    <div className="relative">
      ...
      {/* Navigation */}
      <header className="sticky top-0 z-50 bg-[#FF4D00]/90 backdrop-blur-md border-b border-[#111111]/10 relative">
        <div className="max-w-[1600px] mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <Link href={basePath || '/'} className={`font-serif italic font-black text-4xl absolute left-1/2 -translate-x-1/2 text-[#111111] transition-opacity duration-300 z-10 ${isSearchOpen ? 'opacity-0 sm:opacity-100 pointer-events-none sm:pointer-events-auto' : 'opacity-100'}`}>
            {tLogoUrl ? <img src={tLogoUrl} alt={tBrandName} className="h-8 w-auto object-contain" /> : <div className="flex items-center gap-2"><span>{tBrandName}</span></div>}
          </Link>
...
function Footer({ initialCustomData, basePath }: { initialCustomData?: any, basePath: string }) {
  ...
  const tBrandName = customData?.formData?.brandName || "ATELIER";
  const tLogoUrl = customData?.formData?.logoUrl || "";
  const footerText = customData?.formData?.footerText || "Operating at the intersection of culture, technology, and art.";
  ...
  const copyrightText = customData?.formData?.copyrightText || "© 2026 Atelier Digital.";
  ...
  return (
    <footer className="border-t border-white/10 py-24 px-6 bg-[#111111] mt-auto text-[#EFEFEF]">
      <div className="max-w-[1600px] mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-10">
        <div>
          <div className="font-serif italic font-black text-4xl mb-4 text-[#FF4D00]">{tLogoUrl ? <img src={tLogoUrl} alt={tBrandName} className="h-8 w-auto object-contain" /> : <div className="flex items-center gap-2"><span>{tBrandName}</span></div>}</div>
          <p className="text-xl text-white/50 max-w-sm">{footerText}</p>
        </div>
        <div className="flex flex-col sm:flex-row gap-12 sm:gap-24 text-sm text-white/70">
          <div className="flex flex-col gap-4">
            <span className="font-bold tracking-widest uppercase text-xs mb-2 text-white">{footerCol1}</span>
  ...
  return (
    <CartProvider initialBasePath={basePath} initialCustomData={initialCustomData}>
      <div className="min-h-screen flex flex-col bg-[#F8F7F5] font-body text-[#111111] selection:bg-[#FF4D00] selection:text-white relative">
        {!isAuthPage && <Header initialCustomData={initialCustomData} basePath={basePath !== undefined ? basePath : '/templates/atelier'} />}
        <main className="flex-grow flex flex-col">
          {children}
        </main>
        {!isAuthPage && <Footer initialCustomData={initialCustomData} basePath={basePath !== undefined ? basePath : '/templates/atelier'} />}
        <ToastContainer />
      </div>
    </CartProvider>
  );
