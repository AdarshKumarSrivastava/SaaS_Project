...
function Header({ initialCustomData, basePath }: { initialCustomData?: any, basePath: string }) {
  ...
  const tBrandName = customData?.formData?.brandName || "VANTA";
  ...
  return (
    <div className="relative">
      ...
      {/* Navigation */}
      <header className="sticky top-0 z-50 bg-[#0A0A0A]/80 backdrop-blur-md border-b border-white/[0.05] relative">
        <div className="max-w-[1400px] mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <Link href={basePath || '/'} className={`font-sans font-semibold tracking-tight text-xl absolute left-1/2 -translate-x-1/2 text-white transition-opacity duration-300 z-10 ${isSearchOpen ? 'opacity-0 sm:opacity-100 pointer-events-none sm:pointer-events-auto' : 'opacity-100'}`}>
            {tLogoUrl ? <img src={tLogoUrl} alt={tBrandName} className="h-6 w-auto object-contain" /> : <div className="flex items-center gap-2"><span>{tBrandName}</span></div>}
          </Link>
...
function Footer({ initialCustomData, basePath }: { initialCustomData?: any, basePath: string }) {
  ...
  const tBrandName = customData?.formData?.brandName || "VANTA";
  const tLogoUrl = customData?.formData?.logoUrl || "";
  const footerText = customData?.formData?.footerText || "Pro-level performance meets unprecedented efficiency. Designed in California.";
  ...
  const copyrightText = customData?.formData?.copyrightText || "© 2026 Vanta Systems Inc.";
  ...
  return (
    <footer className="border-t border-white/10 py-16 px-6 bg-black mt-auto text-white">
      <div className="max-w-[1400px] mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-10">
        <div>
          <div className="font-sans font-semibold tracking-tight text-2xl mb-2 text-white">{tLogoUrl ? <img src={tLogoUrl} alt={tBrandName} className="h-6 w-auto object-contain" /> : <div className="flex items-center gap-2"><span>{tBrandName}</span></div>}</div>
          <p className="text-sm text-white/50 max-w-xs">{footerText}</p>
        </div>
        <div className="flex flex-col sm:flex-row gap-8 sm:gap-16 text-[13px] text-white/70">
          <div className="flex flex-col gap-3">
            <span className="font-semibold text-white mb-1">{footerCol1}</span>
  ...
  return (
    <CartProvider initialBasePath={basePath} initialCustomData={initialCustomData}>
      <div className="min-h-screen flex flex-col bg-[#F8F7F5] font-body text-[#111111] selection:bg-[#FF4D00] selection:text-white relative">
        {!isAuthPage && <Header initialCustomData={initialCustomData} basePath={basePath !== undefined ? basePath : '/templates/vanta'} />}
        <main className="flex-grow flex flex-col">
          {children}
        </main>
        {!isAuthPage && <Footer initialCustomData={initialCustomData} basePath={basePath !== undefined ? basePath : '/templates/vanta'} />}
        <ToastContainer />
      </div>
    </CartProvider>
  );
