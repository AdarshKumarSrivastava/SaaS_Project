...
export default function ProductDetailsPage({ params, initialProducts }: { params: Promise<{ id: string }>, initialProducts?: any[] }) {
  ...
  const rawProduct = displayProducts.find((p: any) => p.id === unwrappedParams.id || p.product_id === unwrappedParams.id);
  ...
  const product = {
    ...
    price: rawProduct.base_price || rawProduct.price,
    image: rawProduct.product_images?.[0]?.image_url || rawProduct.three_d_model_url || rawProduct.image || "https://images.unsplash.com/photo-1521572163474-6864f9cf17ab?q=80&w=2000&auto=format&fit=crop",
    category: rawProduct.categories?.category_name || rawProduct.category || "Uncategorized"
  };
  ...
  const productCategory = rawProduct.categories?.category_name || rawProduct.category;

  // Get similar products (same category, excluding current)
  const similarProducts = displayProducts.filter(
    (p: any) => (p.category === productCategory || p.categories?.category_name === productCategory) && (p.id !== unwrappedParams.id && p.product_id !== unwrappedParams.id)
  ).slice(0, 4);
