...
function Header({ initialCustomData, basePath }: { initialCustomData?: any, basePath: string }) {
  ...
  const tBrandName = customData?.formData?.brandName || "AURELIA";
    ...
    <div className="relative">
      ...
      <header className="sticky top-0 z-50 bg-[#F8F7F5]/80 backdrop-blur-md border-b border-black/[0.04] relative">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <Link href={basePath || '/'} className={`font-serif text-3xl tracking-tighter absolute left-1/2 -translate-x-1/2 text-[#111111] transition-opacity duration-300 z-10 ${isSearchOpen ? 'opacity-0 sm:opacity-100 pointer-events-none sm:pointer-events-auto' : 'opacity-100'}`}>
...
function Footer({ initialCustomData, basePath }: { initialCustomData?: any, basePath: string }) {
  ...
  const tBrandName = customData?.formData?.brandName || "AURELIA";
  const tLogoUrl = customData?.formData?.logoUrl || "";
  const footerText = customData?.formData?.footerText || "Defining the modern silhouette with uncompromising luxury and architectural precision.";
  ...
  const copyrightText = customData?.formData?.copyrightText || "© 2026 Aurelia.";
  ...
  return (
    <footer className="border-t border-black/10 py-16 px-6 bg-[#F8F7F5] mt-auto">
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-10">
        <div>
          <div className="font-serif text-2xl tracking-tighter mb-2 text-[#111111] uppercase">{tLogoUrl ? <img src={tLogoUrl} alt={tBrandName} className="h-8 w-auto object-contain" /> : <div className="flex items-center gap-2"><span>{tBrandName}</span></div>}</div>
  ...
  return (
    <CartProvider initialBasePath={basePath} initialCustomData={initialCustomData}>
      <div className="min-h-screen flex flex-col bg-[#F8F7F5] font-body text-[#111111] selection:bg-[#FF4D00] selection:text-white relative">
        {!isAuthPage && <Header initialCustomData={initialCustomData} basePath={basePath !== undefined ? basePath : '/templates/aurelia'} />}
        <main className="flex-grow flex flex-col">
          {children}
        </main>
        {!isAuthPage && <Footer initialCustomData={initialCustomData} basePath={basePath !== undefined ? basePath : '/templates/aurelia'} />}
        <ToastContainer />
      </div>
    </CartProvider>
  );
