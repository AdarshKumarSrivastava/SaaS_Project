...
function Header({ initialCustomData, basePath }: { initialCustomData?: any, basePath: string }) {
  ...
  const tBrandName = customData?.formData?.brandName || "NOIRÉ";
    ...
    <div className="relative">
      ...
      <header className="sticky top-0 z-50 bg-[#F8F7F5]/80 backdrop-blur-md border-b border-black/[0.04] relative">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <Link href={basePath || '/'} className={`font-sans font-light tracking-[0.2em] text-2xl absolute left-1/2 -translate-x-1/2 text-[#2A2A2A] transition-opacity duration-300 z-10 ${isSearchOpen ? 'opacity-0 sm:opacity-100 pointer-events-none sm:pointer-events-auto' : 'opacity-100'}`}>
...
function Footer({ initialCustomData, basePath }: { initialCustomData?: any, basePath: string }) {
  ...
  const tBrandName = customData?.formData?.brandName || "NOIRÉ";
  const tLogoUrl = customData?.formData?.logoUrl || "";
  const footerText = customData?.formData?.footerText || "Scientifically proven, organically sourced. Elevate your daily ritual.";
  ...
  const copyrightText = customData?.formData?.copyrightText || "© 2026 Noiré Skincare.";
  ...
  return (
    <footer className="border-t border-black/10 py-16 px-6 bg-[#F8F7F5] mt-auto">
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-10">
        <div>
          <div className="font-sans font-light tracking-[0.2em] text-xl mb-2 text-[#2A2A2A] uppercase">{tLogoUrl ? <img src={tLogoUrl} alt={tBrandName} className="h-8 w-auto object-contain" /> : <div className="flex items-center gap-2"><span>{tBrandName}</span></div>}</div>
  ...
  return (
    <CartProvider initialBasePath={basePath} initialCustomData={initialCustomData}>
      <div className="min-h-screen flex flex-col bg-[#F8F7F5] font-body text-[#111111] selection:bg-[#FF4D00] selection:text-white relative">
        {!isAuthPage && <Header initialCustomData={initialCustomData} basePath={basePath !== undefined ? basePath : '/templates/noire'} />}
        <main className="flex-grow flex flex-col">
          {children}
        </main>
        {!isAuthPage && <Footer initialCustomData={initialCustomData} basePath={basePath !== undefined ? basePath : '/templates/noire'} />}
        <ToastContainer />
      </div>
    </CartProvider>
  );
