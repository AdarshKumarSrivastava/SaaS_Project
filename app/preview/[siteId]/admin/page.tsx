...
export default function SiteAdminPanel() {
  ...
  useEffect(() => {
    const token = localStorage.getItem(`admin_token_${siteId}`);
  ...
  }, [isAuthenticated, activeTab, siteId]);

  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const res = await apiClient.post(`/sites/${siteId}/admin/login`, { password });
      if (res.token) {
        setIsAuthenticated(true);
        localStorage.setItem(`admin_token_${siteId}`, res.token);
      }
    } catch (err) {
  ...
  const handleLogout = () => {
    localStorage.removeItem(`admin_token_${siteId}`);
