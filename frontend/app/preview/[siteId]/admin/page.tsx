...
import { LayoutDashboard, Users, Settings, LogOut, Package, CreditCard, Activity, ArrowUpRight, TrendingUp } from 'lucide-react';
    ...
    <div className="min-h-screen relative flex bg-[#f4f7f6] overflow-hidden font-sans text-slate-800">
      ...
      <aside className="relative z-10 w-72 bg-white/40 backdrop-blur-2xl border-r border-white/60 p-8 flex flex-col hidden md:flex shadow-[4px_0_24px_-10px_rgba(0,0,0,0.05)]">
        ...
        <nav className="flex-1 space-y-2">
          <a href="#" className="flex items-center gap-3 px-4 py-3 text-sm font-semibold bg-white/60 text-indigo-600 rounded-2xl shadow-sm border border-white/80 transition-all">
            <LayoutDashboard className="w-5 h-5" /> Dashboard
          </a>
          <a href="#" className="flex items-center gap-3 px-4 py-3 text-sm font-medium text-slate-500 hover:text-slate-800 hover:bg-white/40 rounded-2xl transition-all">
            <Package className="w-5 h-5" /> Products & Assets
          </a>
          <a href="#" className="flex items-center gap-3 px-4 py-3 text-sm font-medium text-slate-500 hover:text-slate-800 hover:bg-white/40 rounded-2xl transition-all">
            <Users className="w-5 h-5" /> Demographics
          </a>
          <a href="#" className="flex items-center gap-3 px-4 py-3 text-sm font-medium text-slate-500 hover:text-slate-800 hover:bg-white/40 rounded-2xl transition-all">
            <CreditCard className="w-5 h-5" /> Revenue Stream
          </a>
          <a href="#" className="flex items-center gap-3 px-4 py-3 text-sm font-medium text-slate-500 hover:text-slate-800 hover:bg-white/40 rounded-2xl transition-all">
      ...
      <main className="relative z-10 flex-1 p-10 lg:p-16 overflow-y-auto h-screen">
        <header className="mb-12 flex justify-between items-end">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }}>
            ...
            <p className="text-sm font-medium text-slate-500 mt-2">Real-time metrics for your generated environment.</p>
          </motion.div>
