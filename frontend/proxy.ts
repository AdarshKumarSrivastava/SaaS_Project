...
export async function proxy(request: NextRequest) {
  ...
  if (hostname === 'localhost:3000' || hostname === '127.0.0.1:3000') {
    ...
    if (liveMatch) {
      ...
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/api/sites/${siteId}/live`);
        if (res.ok) {
  ...
  if (subdomain && !['www', 'app', 'admin'].includes(subdomain)) {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/api/sites/live/${subdomain}`);
      if (res.ok) {
        const data = await res.json();
